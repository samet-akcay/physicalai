...
Therefore Runtime currently has two construction paths:

```text
Python API / Config / instantiate_obj
    -> Runtime utility functions
    -> custom parsing and recursive construction
...
`Config.from_dict` or `instantiate_obj`.
...
during migration. Runtime should stop using `FromConfig` and `instantiate_obj`
internally once their call sites have migrated. They should delegate to
package-owned parser builders and jsonargparse, rather than implement another
parser or recursive constructor.
...
These adapters must not perform constructor signature parsing or object
construction. Once a recipe has been normalized and authorized, jsonargparse
should validate it against the expected component base type and instantiate it.
...
| `serializable.py`                | Dataclass conversion and type reconstruction               |
| `_yaml.py`                       | YAML loading and saving                                    |
| `_envelope.py`                   | Transport envelope validation                              |
| `inference/component_factory.py` | A second recursive component instantiator                  |
| `cli/run.py`                     | jsonargparse parsing and instantiation                     |

The overlap creates multiple paths with different behavior:

```text
Config.instantiate()            -> custom strict instantiator
...
instantiate_component()        -> inference-specific custom instantiator
```
...
### Phase 3: replace typed Config serialization

Move typed configuration classes to plain dataclasses or Pydantic models.
...
Use package parser builders for load, save, and construction.

Keep `Config` as a compatibility facade until downstream packages migrate.
...
6. Delete recursive construction and import code from
   `inference/component_factory.py`.
...
```text
physicalai/config/_instantiate.py
physicalai/config/loading.py recursive constructor logic
physicalai/config/mixin.py signature parsing
physicalai/config/serializable.py type reconstruction
